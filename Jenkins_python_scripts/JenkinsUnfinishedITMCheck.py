#!/usr/bin/env python3
# -*- coding=utf-8 -*-
"""根据输入的ITM单路径及ITM单号,查找出ITM单中与ITM单路径下所有单的文件冲突关系,如果有冲突
则提示冲突单并退出,如果没有冲突则合并输入单号的列表为新列表,提交生成增量"""
import pandas as pd
...
class CheckChangeFile:
    def getAllFiles(self, path, filename):
        """
        返回path下一级所有的filename的全路径
        :param self:
        :param path:需要扫描的目录列表
        :return:ITM单号及文件全路径列表
        """
        filecollections = []
        for itm in os.listdir(path):
            if os.path.isfile(os.path.join(path, itm)):
                continue  # 目录下是文件的直接路过
            pathfile = os.path.join(path, itm, filename)
            if os.path.isfile(pathfile):
                filecollections.append((itm, pathfile))
            else:
                filecollections.append((itm, ''))
    ...
    def cheConflict(self, svnpath, itms, filename):
        '''查询ITM所中所有与其冲突的文件
        svnpath为未完成的ITM单本地路径,itms为本次补丁的单字符串以","分隔.file变更列表文件名
        返回ITM合并列表以及冲突列表'''
        filecollections = self.getAllFiles(svnpath, filename)
        byzlist = pd.DataFrame(columns=("itm", "file"))
        syzlist = pd.DataFrame(columns=("itm", "file"))
        conflict = pd.DataFrame(columns=("itm_x", "itm_y", "file"))
        itmlist = itms.split("#")
        nofileitm = []  # 本次测试单中,没有变更列表的ITM单
        nofileitmlist = []  # SVN待测试目录下变更列表内容为空的ITM单

        for i in range(len(itmlist) - 1, -1, -1):  # 倒序遍历不会对list删除后找不到索引的问题
            itm = itmlist[i]
            # print("-------%s" % itm)
            if not os.path.isfile(os.path.join(svnpath, itm, filename)):
                # print("---------- %s " % os.path.join(svnpath, itm, filename))
                nofileitm.append(itm)
                itmlist.pop(i)
        if len(itmlist) == 0:
            print(Fore.RED + " ---------本次构建的ITM单【%s】没有变更列表文件,请确认，无需进行冲突检查..." % itms)
            return syzlist, conflict
        for itmname, file in filecollections:
            if (itmname in itmlist and file != ""):
                syzlist = syzlist.append(self.getListFileContext(itmname, file))  # 要测试ITM单文件列表
            elif file != "":
                byzlist = byzlist.append(self.getListFileContext(itmname, file))  # 其他未完成要比较的列表
            else:
                nofileitmlist.append(itmname)  # SVN目录下没有变更列表的ITM单
        for nitem in nofileitmlist:
            print(Fore.CYAN + "发现未完成的ITM单【%s】中不存在【%s】,请检查是否正常!" % (nitem, filename))

        # conflict=byzlist[byzlist.file.isin(syzlist["file"])]
        conflict = pd.merge(syzlist, byzlist, on="file")  # 内连接找出冲突的文件
        return syzlist, conflict  # 返回合并单列表和冲突列表
...
if __name__ == "__main__":
    ...
    if flag == "1":  # 1:财补等jar,war包按全量出的会合并,其他运政增量出只提示),参数为1时必须指定参数5,参数6,参数7,参数8
        ...
        if not os.path.isdir(filepath):
            print(Fore.RED + "找不到未完成ITM单目录【%s】,退出测试" % filepath)
            sys.exit(1)
        ...
        if not os.path.isdir(controlledpath):
            ...
            sys.exit(1)
    ccf = CheckChangeFile()
    syzlist, conflict = ccf.cheConflict(filepath, itmno, filename)
    for index, item in conflict.iterrows():  # 打印冲突明细
        try:
            print(Fore.RED + "冲突原单【%s】，冲突单【%s】,冲突文件【%s】" % (item["itm_x"], item["itm_y"], item["file"]))
        except UnicodeDecodeError as e:
            print(item)
    if flag == "1":  #
        if len(conflict) >= 1:
            print(Fore.RED + "\n*****错误*****存在冲突,请根据冲突单明细合并ITM单,重新进行构建!!")
            sys.exit(1)
        else:
        ...
        if len(conflict) >= 1:
            print(Fore.YELLOW + "***警告**存在冲突,请与开发沟通是否影响发布***警告**")
        else:
